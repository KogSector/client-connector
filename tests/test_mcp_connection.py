...
def test_mcp_compression():
    ...
    try:
        ...
        if config_resp.status_code == 404:
            ...
            sse_url = (
                servers[server_key]["url"]
                if server_key
                else f"http://localhost:3020/api/v1/mcp/sse?agent_id={AGENT_ID}"
            )
                        ...
                        try:
                            ...
                            if "result" in data:
                                if data.get("id") != tool_call_id:
                                    print(
                                        f"Ignoring result for id {data.get('id')}, expecting {tool_call_id}"
                                    )
                                    continue
    ...
    if got_endpoint.wait(timeout=10):
        ...
        try:
            ...
            requests.post(
                post_url, json=initialized_req, headers={"Content-Type": "application/json"}
            )
            time.sleep(0.5)

            response = requests.post(
                post_url, json=tool_call_request, headers={"Content-Type": "application/json"}
            )
